...
class Residual3DFlow:
    ...
    @torch.no_grad()
    def compute_residual_3d_flow(self, raft_flow, depth_images, poses):
        """
        raft_flow: (N, H, W, 2)
        ...
        geometric_coords_3d_1, invalid_mask_g = self.unproject_and_transform(depth_images[:-1], pixel_coords_flattened, poses)    # (N, H*W, 3) 
    ...
    @torch.no_grad()
    def unproject_and_transform(self, depth_images, pixel_coords_flattened, poses):
        """
        depth_images: (N, H, W)
        pixel_coords_flattened: (H*W, 2)
        poses: (N+1, 4, 4)
        """
        ...
        norm_cam_coords = self.concat_ones_last_axis(norm_cam_coords).view(1, -1, 3)                        # (1, H*W, 3)
        ...
        return (coords_3d_h_1.permute(0, 2, 1))[..., :3], invalid_mask                                      # (N, 4, H*W) -> (N, H*W, 4) -> (N, H*W, 3)   
        

    def concat_ones_last_axis(self, tensor):
        return torch.concatenate((tensor, torch.ones((*tensor.shape[:-1], 1), dtype=torch.float32, device=self.device)), dim=-1)
